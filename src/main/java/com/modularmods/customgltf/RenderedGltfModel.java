...
public class RenderedGltfModel {
		...
		public void initMaterialCommand(List<Runnable> gltfRenderData, RenderedGltfModel renderedModel, MaterialModel materialModel) {
			...
			if(materialModel instanceof MaterialModelV2) {
				...
				if(baseColorTexture == null) {
					...
					else colorMap = CustomGLTF.getInstance().getDefaultColorMap();
				}
				...
				if(normalTexture == null) {
					...
					else normalMap = CustomGLTF.getInstance().getDefaultNormalMap();
				}
				...
				if(specularTexture == null) {
					...
					else specularMap = CustomGLTF.getInstance().getDefaultSpecularMap();
				}
				...
				normalMap = normalTexture == null ? CustomGLTF.getInstance().getDefaultNormalMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(normalTexture.index));
				specularMap = specularTexture == null ? CustomGLTF.getInstance().getDefaultSpecularMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(specularTexture.index));
	...
	public int obtainGlTexture(List<Runnable> gltfRenderData, TextureModel textureModel) {
		...
		if(glTexture == null) {
			...
			if (pixelData == null)
			{
				CustomGLTF.logger.warn("Could not extract pixel data from image");
